...
public class Broadcaster {
    ...
    private final IntentService source;

    public Broadcaster(IntentService source) {
        this.source = source;
    }

    public void broadcastFullUpdateNotStarted() {
        Intent broadcastIntent = new Intent();
        broadcastIntent.setAction(FULL_UPDATE_NOT_STARTED);
        broadcastIntent.addCategory(Intent.CATEGORY_DEFAULT);
        broadcastIntent.putExtra(EXTRA_MESSAGE, "Full update service not started, disconnected to server.");
